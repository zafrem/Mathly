...
export default function PracticeView({ params }: { params: Promise<{ type: string }> }) {
  ...
  }, [isFinished, score, userName, type]);

  const handleSuccess = (timeMs: number) => {
    if (isFinished) return;
    
    // Scoring logic: (10000 / timeMs) * digits_modifier
    // Faster = more points. 1s = 100 points * digits.
    const points = Math.max(1, Math.floor((10000 / Math.max(timeMs, 100)) * digits));
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <h1 className="text-4xl font-black text-gray-800 mb-2">Sprint Over!</h1>
          <p className="text-gray-500 mb-8">{userName}, you scored:</p>
          
          <div className="text-6xl font-black text-blue-500 mb-12 tabular-nums">
          ...
          <div className="flex flex-col gap-4">
            <button onClick={() => window.location.reload()} className="w-full py-4 bg-blue-500 text-white rounded-2xl font-bold text-lg hover:bg-blue-600 transition-colors flex items-center justify-center gap-2">
              <RotateCcw size={20} /> Try Again
            </button>
            <button onClick={() => router.push('/')} className="w-full py-4 bg-gray-100 text-gray-600 rounded-2xl font-bold text-lg hover:bg-gray-200 transition-colors">
              Back to Home
            </button>
          </div>
        </motion.div>
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        <header className="flex items-center justify-between mb-12">
          <button onClick={() => router.push('/')} className="p-3 rounded-full bg-white shadow-sm border border-gray-100 text-gray-600 hover:bg-gray-50 transition-colors">
            <ArrowLeft size={24} />
          </button>
        ...
        <main className="relative">
          <AnimatePresence mode="wait">
            {countdown > 0 ? (
              ...
              <motion.div key="practice-content" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <div className="relative">
                  <AnimatePresence>
                    {showCelebration && (
                      <motion.div initial={{ opacity: 0, scale: 0.5, y: 0 }} animate={{ opacity: 1, scale: 1.2, y: -100 }} exit={{ opacity: 0 }} className="absolute inset-x-0 flex justify-center pointer-events-none z-10">
                ...
                <div className="text-center mb-12">
                  <h1 className="text-3xl font-black text-gray-800 capitalize mb-2">{type} Practice</h1>
